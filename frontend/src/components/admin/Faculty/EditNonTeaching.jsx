...
const NonTeachingStaffForm = ({ isOpen, onClose, setRefresh, faculty }) => {
    if (!isOpen) return null; // Don't render the modal if it's not open

    const [departmentName, setDepartmentName] = useState(faculty?.department_name || "");
    const [facultyName, setFacultyName] = useState(faculty?.staff_name || '');
    const [designation, setDesignation] = useState(faculty?.designation || '');
    const [email, setEmail] = useState(faculty?.email || '');
    const [additionalInfo, setAdditionalInfo] = useState(faculty?.additional_info || '');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            await axios.put(`${apiUrl}/api/faculty/non_teaching/${faculty.id}`, formData, {
                headers: {
